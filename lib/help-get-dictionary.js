...
var helpGetAllSync = require("./help-get-all-sync");
 ...
 * @private
 */
module.exports = function helpGetDictionary(options, cb) {
  // Defaults
    ...
    _.each(files, function (module, filename) {
      ...
      if (options.aggregate) {
        ...
        if (!_.isPlainObject(module)) {
          throw new Error(
            "When using `aggregate`, modules must export dictionaries.  But module (`" +
              filename +
              "`) is invalid:" +
              module
          );
      ...
      if (!options.dontLoad && options.identity !== false) {
        ...
        if (!module.identity) {
          if (options.replaceExpr) {
            module.identity = filename.replace(
              options.replaceExpr,
              options.replaceVal
            );
          } else {
        ...
        keyName = options.useGlobalIdForKeyName
          ? module.globalId
          : module.identity;
      }
      ...
      if (dictionary[keyName] && !options.allowDuplicateKeys) {
        var e = new Error(
          "Duplicate filename detected: `include-all` attempted to load two files named `" +
            keyName +
            "` (case-insensitive)."
        );
