...
module.exports = function includeAll(options) {
  ...
  if (typeof options.flattenDirectories !== "undefined") {
    throw new Error(
      "As of include-all v1.0.1, `flattenDirectories` was replaced with `flatten`.  See https://github.com/balderdashy/include-all#options for more information."
    );
  }
  ...
  var modules = (function _recursivelyIncludeAll(thisDirname, _depth) {
    ...
    try {
      ...
      if (options.optional) {
        ...
        var dirNotFoundErr = new Error(
          "`include-all` could not scan directory (`" +
            thisDirname +
            "`) could not be scanned for files.\nDetails:" +
            e.stack
        );
        ...
        if (options.flatten) {
          _.each(descendantModules, function (rhs, grandchildKey) {
            if (options.keepDirectoryPath) {
              _modules[
                path.join(path.basename(relativePath), grandchildKey)
              ] = rhs;
            } else {
              if (_modules[grandchildKey]) {
                throw new Error(
                  "Attempting to flatten modules but duplicate key detected (`" +
                    grandchildKey +
                    "`).  Enable `keepDirectoryPath: true` to enable namespacing based on hierarchy."
                );
              }
